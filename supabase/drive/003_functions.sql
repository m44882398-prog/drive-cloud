...
create or replace function public.calculate_project_size(project_uuid uuid) returns bigint language sql stable as $$ select coalesce(sum(size), 0) from public.files where project_id = project_uuid and not is_trashed $$;
